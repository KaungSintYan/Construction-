<template>  
    <div class="container-fluid"  @click="$store.commit('setActive', false)">    
        ...
        <div class="row mx-2">   
            ...
            <el-table-column prop="" :label="$t('exit_date')" width="200" sortable />          
...
export default {
    ...
    methods:{ 
        async getData(val){            
            let type= "";
            let api= '/materialmanagement/outbound_order_appcc';            
            if(val == "CC"){               
                type = 0;
        ...
        getEdit(data){
        this.FormData.outbound_order_id= data.outbound_order_id;
        ...
        async updateData() {
            let response = await authApi({
                method: 'POST',
                api: '/materialmanagement/outbound_order_appcc',
                params: {
                       id: '',
                       type: 1,
                },
