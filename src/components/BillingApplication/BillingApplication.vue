<template>  
  <div v-loading="loading" class="loading__div" v-if="loading"></div>   
    <div class="container-fluid"  @click="$store.commit('setActive', false)" v-if="!loading"> 
        <div class="row">
            <FinancialSearching :api="apiName" :advanced="advanced"  :routeName="routeName" :createBtn="createBtn" :showBillingApplication="showBillingApplication"           
        ...
        <div class="row mx-2">    
         <el-table :data="data" border style="width: 100%" @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="receiver" :label="$t('receiver')" width="200" sortable />
            <el-table-column prop="invoice_type" :label="$t('invoice_type')" width="200" sortable />
            <el-table-column prop="billing_status" :label="$t('billing_status')" width="200" sortable />          
            <el-table-column prop="invoice_number" :label="$t('invoice')" width="200" sortable />          
            <el-table-column prop="tax_rate" :label="$t('tax_number')" width="200" sortable />          
            ...
            <template #default="scope">
             <div class="action__btn d-flex">
                            <router-link :to="`/billing_application/edit/${scope.row.billing_application_id}`"
                            ...
                            placement="top"
                            >
                            <button class="btn-danger circle__btn" @click="deleteAll(scope.row.billing_application_id)" v-show="deleteBtn && scope.row.editable">
                            ...
                            placement="top"
                            >
                            <button class="btn-warning circle__btn"  @click="showApprover(scope.row.billing_application_id)">
...
export default {
    ...
    data() {
        return {         
             //searching
            apiName: '/financialmanagement/get_billing_application',
            advanced: true,            
            routeName: "/billing_application/",
            showBillingApplication: true,         
            //permission
            showBtn: false,
            createBtn: false,
            updateBtn: false,
    ...
    methods:{  
        ...
        async showApprover(id){ 
            let response = await authApi({
                method: 'GET',
                api: '/financialmanagement/get_billing_application_approve_cc',
                params: {
                    billing_application_id: id,                  
                }   
            });          
        ...
        async deleteAll(id = null){            
            ...
            if(id != null && id != ""){
                ...
                for(let i=0; i< this.selectedData.length; i++){
                     list.push(this.selectedData[i].billing_application_id);   
                }
            }           
            if(list.length > 0){
                let response = await authApi({
