    ...
    <div class="container-fluid"  @click="$store.commit('setActive', false)" v-if="!loading">
        <div class="row mb-3">
            ...
            <div class="col-sm-8 d-flex justify-content-end">
                <el-select id="customer_status" class="mr-2" v-model="customer_status" :placeholder="$t('please_choose')"  style="width: 100%">
                    <el-option label="Customer"  value="Customer" />  
                    <el-option  label="Supplier"  value="Supplier" />  
                </el-select> 
                <el-select id="approval_status" class="mr-2"  v-model="approval_status" :placeholder="$t('please_choose')"  style="width: 100%">
                    <el-option label="Approver"  value="Approver" />  
                    <el-option  label="CC"  value="CC" />  
                </el-select>                
                <el-button type="primary" @click="getData()">
                    <el-icon class="el-icon--left"><font-awesome-icon icon="fa-solid fa-search" /></el-icon>Search
                </el-button>                    
            </div> 
        </div>
        <div class="row mx-2">           
        <el-table :data="data" border style="width: 100%">
            ...
            <el-table-column prop="revenue_contract_id" :label="$t('revenue_contract')" width="200" sortable v-if="!showClientTable" />
            <el-table-column prop="spending_contract_id" label="Spending Contract" width="200" sortable v-if="showClientTable" />
            ...
            <el-table-column label="Approver" width="200">
                <template #default="scope">
                    <div class="flex flex-wrap gap-2">
                        <el-tag
                          v-for="item in scope.row.approver"
                          :key="item"
                          type=""
                          class="mx-1"
                          effect="light"
...
import $ from 'jquery';
export default {
    name: 'MarginRegistrationApprover',
    data() {
        return {             
            data: [],
            loading: false,
            FormData: {
              margin_registration_id: "",
              remark: "",
              approval_status: ""
            },  
            approval: [
    ...
    methods:{ 
        async getData(val){            
            let type= "";
            let api = "";
            // if(this.showApprover && this.showClientTable ){
            //     api= '/financialmanagement/client_margin_approve_cc';
            //     type= 1;
            // }  
            // if(!this.showApprover && !this.showClientTable){
            //     api= '/financialmanagement/supplier_margin_approve_cc';                
            //     type=0;
            // }    
                
            if(this.customer_status == "Supplier"){
                api= '/financialmanagement/supplier_margin_approve_cc';   
                this.showClientTable = true;
                if(this.approval_status == "CC" ){               
                type = 0;              
        ...
        getEdit(data){
        this.FormData.margin_registration_id= data.margin_registration_id;
