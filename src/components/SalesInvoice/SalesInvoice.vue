<template>  
    ...
    <div class="container-fluid"  @click="$store.commit('setActive', false)" v-if="!loading"> 
        <div class="row">
            <FinancialSearching :api="apiName" :advanced="advanced"  :routeName="routeName" :createBtn="createBtn" :showSaleInvoice="showSaleInvoice"           
         ...
         <el-table :data="data" border style="width: 100%" @selection-change="handleSelectionChange">
            ...
            <template #default="scope">
             <div class="action__btn d-flex">
                            <router-link :to="`/sales_invoice/show/${scope.row.sales_invoice_id}`"
                                v-show="showBtn">
                                    <el-tooltip
                                        class="box-item"
                                        effect="dark"
                            ...
                            <!-- <router-link :to="`/sales_invoice/edit/${scope.row.sales_invoice_id}`"
                            ...
                            placement="top"
                            >
                            <button class="btn-danger circle__btn" @click="deleteAll(scope.row.sales_invoice_id)" v-show="deleteBtn && scope.row.editable">
                            ...
                            placement="top"
                            >
                            <button class="btn-warning circle__btn"  @click="showApprover(scope.row.sales_invoice_id)">
...
export default {
    ...
    data() {
        return {  
            //searching
            apiName: '/financialmanagement/get_sales_invoice',
            advanced: true,            
            routeName: "/sales_invoice/",
            showSaleInvoice: true, 
            //endsearching          
            data: [], 
            loading: false,
            selectedData: [],
    ...
    methods:{    
        ...
        async getData(){ 
            ...
            if(response.message){            
               ...
               this.data= list; 
                console.log('saleInvoice>>'+ JSON.stringify(this.data));
        ...
        async showApprover(id){ 
            let response = await authApi({
                method: 'GET',
                api: '/financialmanagement/get_sales_invoice_approve_cc',
                params: {
                    sales_invoice_id: id,                  
                }   
            });          
        ...
        async deleteAll(id = null){          
            ...
            if(id != null && id != ""){
                ...
                for(let i=0; i< this.selectedData.length; i++){
                    list.push(this.selectedData[i].sales_invoice_id);   
                }
            } 
            if(list.length > 0){
                let response = await authApi({
                ...
                if(response.message){
                    ...
                    this.data=[];
                    this.getData();
                    toast.success("Sale Invoice Delete successful !", {
