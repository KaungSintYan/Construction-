<template>  
    <div class="container-fluid"  @click="$store.commit('setActive', false)">
        <div class="row">
            <FinancialSearching :api="apiName" :advanced="advanced"  :routeName="routeName" :createBtn="createBtn" :showSecurityDeposit="showSecurityDeposit"           
        ...
        <div class="row mx-2">    
         ...
         <el-table :data="data" border style="width: 100%" @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="margin_type" :label="$t('margin')" width="200" sortable />
            <!-- <el-table-column prop="margin_nature" :label="$t('margin_nature')" width="200" sortable /> -->
            <el-table-column prop="margin_amount" :label="$t('margin_amt')" width="200" sortable />
            <el-table-column prop="return_date" :label="$t('return_date')" width="200" sortable />
            ...
            <template #default="scope">
             <div class="action__btn d-flex">
                            ...
                            <router-link :to="`/security_deposit_refund/show/customer/${scope.row.security_deposit_refund_id}`"
                                v-show="showBtn">
                                    <el-tooltip
                                        class="box-item"
                                        effect="dark"
                            ...
                            <!-- <router-link :to="`/security_deposit_refund/edit/${scope.row.security_deposit_refund_id}`"
                            ...
                            placement="top"
                            >
                            <button class="btn-danger circle__btn" @click="deleteAll(scope.row.security_deposit_refund_id)" v-show="deleteBtn && scope.row.editable">
                            ...
                            placement="top"
                            >
                            <button class="btn-warning circle__btn"  @click="showApprover(scope.row.security_deposit_refund_id)">
...
export default {
    ...
    data() {
        return { 
            //searching
            apiName: '/financialmanagement/get_security_deposit_refund',
            advanced: true,            
            routeName: "/security_deposit_refund/",
    ...
    methods:{ 
        ...
        async showApprover(id){ 
            let response = await authApi({
                method: 'GET',
                api: '/financialmanagement/get_security_deposit_approve_cc',
                params: {
                    security_deposit_refund_id: id,                  
                }   
            });          
        ...
        async deleteAll(id = null){        
            ...
            if(id != null && id != ""){
                ...
                for(let i=0; i< this.selectedData.length; i++){
                    list.push(this.selectedData[i].security_deposit_refund_id);   
                }
            }       
            if(list.length > 0){
                let response = await authApi({
                ...
                if(response.message){                  
                    this.getData();
                    toast.success("Security Deposit Refund Delete successful !", {
