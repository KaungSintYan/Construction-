    ...
    <div class="container-fluid"  @click="$store.commit('setActive', false)" v-if="!loading"> 
        ...
        <div class="row mx-2">
            <el-table :data="data" border style="width: 100%" @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="material_inventory_name" :label="$t('material_name')" width="250" />         
                <el-table-column prop="measure_unit" :label="$t('measure_unit')" width="250" />         
                <el-table-column prop="specification" :label="$t('specification')" width="250" />  
                <el-table-column prop="brand" :label="$t('brand')" width="250" />  
                ...
                <el-table-column prop="other_property" :label="$t('other_property')" width="250" />         
                <el-table-column prop="instock_amount" label="Instock Amount" width="250" />  
                <el-table-column prop="remark" :label="$t('remark')" width="250" />         
                ...
                <el-table-column fixed="right" :label="$t('operate')"  width="300" >
                    <template #default="scope">
                        <router-link :to="`/materialinventory/edit/${scope.row.inventoryid}`" v-show="updateBtn">
                        ...
                        <el-button type="button" class="btn btn-sm btn-danger mr-2" @click="deleteAll(scope.row.inventoryid)" v-show="deleteBtn">
...
export default {
    ...
    methods:{
        ...
        checkOne(id, isSelected){         
            this.data.find(x => x.inventoryid === id).isSelected = isSelected;    
            if(isSelected){
                this.selectedCount= this.selectedCount +1;
            }      
            else{
        ...
        async deleteAll(id = null){          
            ...
            if(id != null && id != ""){
                ...
                for(let i=0; i< this.data.length; i++){
                    if(this.data[i].isSelected){
                        list.push(this.data[i].inventoryid);                    
                    }
                }
            } 
            ...
            if(list.length > 0){
                ...
                if(response.message){
                    ...
                    this.data=[];
                    this.getData();
                    toast.success("Material Inventory Delete successful !", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 3000,
                    });
                }                         
