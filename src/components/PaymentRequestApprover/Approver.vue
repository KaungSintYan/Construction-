<template>  
    <div class="container-fluid"  @click="$store.commit('setActive', false)">    
        ...
        <div class="row mx-2">   
            ...
            <el-table-column prop="" :label="$t('payment_status')" width="200" sortable />
...
export default {
    ...
    methods:{ 
        async getData(val){            
            let type= "";
            let api= '/financialmanagement/payment_request_approve_cc';            
            if(val == "CC"){               
                type = 0;
        ...
        getEdit(data){
        this.FormData.payment_request_id= data.payment_request_id;
        ...
        async updateData() {
            let response = await authApi({
                method: 'POST',
                api: '/financialmanagement/payment_request_approve_cc',
                params: {
                       id: '',
                       type: 1,
                },
