                            ...
                            <div class="form-group mb-3">
                                <div class="form-group mb-3">
                                    <label for="report_date"> 
                                        <span class="text-danger">*</span> <small><b> {{ $t('reporting_date') }} </b></small>            
                                    </label>
                                </div>                        
                                <el-date-picker v-model="FormData.report_date"  id="check_date" type="date" 
                                format="YYYY-MM-DD" value-format="YYYY-MM-DD" placeholder="Pick a Date" size="default"  style="width: 100%;" editable clearable  />
                            </div>  
                        </div>                     

                        <div class="form-group mb-3">
                            <label for="today_work_content">
                                <small><b> {{ $t('tdy_work_content') }} </b></small>
                            </label>
                            <el-input v-model="FormData.work_content" maxlength="1000" 
                            :placeholder="$t('please_enter')"  show-word-limit type="textarea" />  
                        </div>

                        <div class="form-group mb-3">
                            <label for="unfinished_work_today">
                                <small><b> {{ $t('unfinished_work_tdy') }} </b></small>   
                            </label>
                            <el-input v-model="FormData.unfinished_work" maxlength="1000" 
                            :placeholder="$t('please_enter')"  show-word-limit type="textarea" />  
                        </div>
                        
                        <div class="form-group mb-3">
                            <label for="work_plan_for_tomorrow">
                                <small><b> {{ $t('work_plan_tmr') }} </b></small>
                            </label>
                            <el-input v-model="FormData.plan_for_tomorrow" maxlength="1000" 
                            :placeholder="$t('please_enter')"  show-word-limit type="textarea" />  
                        </div>
      ...
      methods:{       
        ...
        async getData(){
            let response = await authApi({
                 method: 'POST',
                 api: '/constructionmanagement/get_daily_report',               
                 params: {
         ...
         async updateData(e){   
             ...
             let response = await authApi({
                 method: 'PUT',
                 api: '/constructionmanagement/daily_report',
                 data: this.FormData 
             });
             if(response.status == 200){     
                ...
                this.appendixImages.forEach((value) => {
                 ...
                 method: 'PUT',
                 api: '/constructionmanagement/daily_report_datafiles',       
                 data: formData
                 });
                 if(imgResponse.status == 200){                    
                     toast.success("Darily Report Update successful !", {
