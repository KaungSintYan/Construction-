<template>  
    ...
    <div class="container-fluid"  @click="$store.commit('setActive', false)" v-if="!loading"> 
        <div class="row">
            ...
            :showRepaymentNote="showRepaymentNote"
        ...
        <el-table :data="data" border style="width: 100%" @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="repayer" :label="$t('repayer')" width="200" sortable />
            <el-table-column prop="repayment_date" :label="$t('repayment_date')" width="200" sortable />
            <el-table-column prop="total_payment_amount" :label="$t('total_repayment_amount')" width="220" sortable />
            <el-table-column prop="ordinary_repayment_amount" :label="$t('ordinary_repayment_amount')" width="270" sortable />   
            <el-table-column prop="reimbursement_offset_amount" :label="$t('reimbursement_offset_amt')" width="290" sortable />   
            ...
            <template #default="scope">
             <div class="action__btn d-flex">
                            ...
                            <router-link  :to="`/repayment_note/show/${scope.row.repayment_note_id}`"
                                v-show="showBtn">
                                <el-tooltip
                                    class="box-item"
                                    effect="dark"
                            ...
                            <!-- <router-link :to="`/repayment_note/edit/${scope.row.repayment_note_id}`"
                            ...
                            placement="top"
                            >
                            <button class="btn-danger circle__btn" @click="deleteAll(scope.row.repayment_note_id)" v-show="deleteBtn && scope.row.editable">
                            ...
                            placement="top"
                            >
                            <button class="btn-warning circle__btn"  @click="showApprover(scope.row.repayment_note_id)" >
...
export default {
    ...
    data() {
        return {     
            //searching
            apiName: '/costmanagement/get_repayment_note',
            advanced: true,            
            routeName: "/repayment_note/",   
            showRepaymentNote: true,        
            //endsearching
    ...
    methods:{     
        ...
        async showApprover(id){ 
            let response = await authApi({
                method: 'GET',
                api: '/costmanagement/get_repaymentnote_approve_cc',
                params: {
                    repayment_note_id: id,                  
                }   
            });          
            ...
            if(id != null && id != ""){
                ...
                for(let i=0; i< this.data.length; i++){
                    if(this.data[i].isSelected){
                        list.push(this.data[i].revenue_contract_visa_id);                    
                    }
                }
            }           
            if(list.length > 0){
