   ...
   <div v-loading="loading" class="loading__div" v-if="loading"></div>   
    <div class="container-fluid"  @click="$store.commit('setActive', false)" v-if="!loading"> 
        <div class="row">
            ...
            :showexpenditureSettlement="showexpenditureSettlement"
        ...
        <div class="row mx-2">  
         <el-table :data="data" border style="width: 100%" @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="settlement_number" :label="$t('odd_number')" width="200" sortable />
            <el-table-column prop="contract_statement_name" :label="$t('contract_statement_name')" width="220" sortable />
            <el-table-column prop="settlement_amount" :label="$t('settlement_amount')" width="200" sortable />
            <el-table-column prop="settlement_date" :label="$t('settlement_date')" width="200" sortable />
            <el-table-column prop="settlement_content" :label="$t('settlement_content')" width="200" sortable />       
            ...
            <template #default="scope">
             <div class="action__btn d-flex">
                            <!-- <router-link :to="`/expenditureSettlement/edit/${scope.row.spending_contract_settlement_id}`"
                            ...
                            <router-link :to="`/expenditureSettlement/show/${scope.row.spending_contract_settlement_id}`"
                            v-show="showBtn">
                                <el-tooltip
                                    class="box-item"
                                    effect="dark"
                            ...
                            placement="top"
                            >
                            <button class="btn-danger circle__btn" @click="deleteAll(scope.row.spending_contract_settlement_id)" v-show="deleteBtn && scope.row.editable">
                            ...
                            placement="top"
                            >
                            <button class="btn-warning circle__btn"  @click="showApprover(scope.row.spending_contract_settlement_id)">
        ...
        <div class="modal-dialog modal-lg">         
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel"> <strong> {{ $t(' Approver List ') }}</strong> </h5>
                </div>               
...
export default {
    ...
    data() {
        return {    
             ...
             //searching
            apiName: "/contractmanagement/get_spending_contract_settlement",
            advanced: true,            
            routeName: "/expenditureSettlement/",
            showexpenditureSettlement: true,
            //endsearching
            //
            data: [], 
            loading: false,
    ...
    methods:{   
        ...
        async getData(){   
            ...
            let response = await authApi({
                method: 'POST',
                api: '/contractmanagement/get_spending_contract_settlement',
                params: {
                       id: '',
                       keyword: '',             
        ...
        async showApprover(id){ 
            let response = await authApi({
                method: 'GET',
                api: '/contractmanagement/get_scs_approve_cc',
                params: {
                    settlement_id: id,                  
                }   
            });          
         ...
         async deleteAll(id = null){          
            ...
            if(id != null && id != ""){
                ...
                for(let i=0; i< this.selectedData.length; i++){
                    list.push(this.selectedData[i].spending_contract_settlement_id);         
                }
            } 
            ...
            if(list.length > 0){
                ...
                if(response.message){
                    ...
                    this.getData();
                    toast.success("RevenueContractExecution Delete successful !", {
