         ...
         <el-table :data="data" border style="width: 100%" @selection-change="handleSelectionChange">
            ...
            <template #default="scope">
             <div class="action__btn d-flex">
                            <!-- <router-link :to="`/revenueSettlement/edit/${scope.row.revenue_contract_settlement_id}`"
                            ...
                            <router-link :to="`/revenueSettlement/show/${scope.row.revenue_contract_settlement_id}`"
                            v-show="showBtn">
                                <el-tooltip
                                    class="box-item"
                                    effect="dark"
                            ...
                            placement="top"
                            >
                            <button class="btn-danger circle__btn" @click="deleteAll(scope.row.revenue_contract_settlement_id)" v-show="deleteBtn && scope.row.editable">
                            ...
                            placement="top"
                            >
                            <button class="btn-warning circle__btn"  @click="showApprover(scope.row.revenue_contract_settlement_id)">
...
export default {
    ...
    data() {
        return { 
               //searching
            apiName: "/contractmanagement/get_revenue_contract_settlement",
            advanced: true,            
            routeName: "/revenueSettlement/",
            showSettlement: true,
            //endsearching
    ...
    methods:{   
        ...
        async deleteAll(id = null){          
            ...
            if(id != null && id != ""){
                ...
                for(let i=0; i< this.selectedData.length; i++){
                    list.push(this.selectedData[i].revenue_contract_settlement_id);         
                }
            } 
        ...
        async showApprover(id){ 
            let response = await authApi({
                method: 'GET',
                api: '/contractmanagement/get_rcs_approve_cc',
                params: {
                    settlement_id: id,                  
                }   
            });          
