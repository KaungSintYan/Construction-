<template>
    <div class="form-group mb-3">
        ...
        <el-input type="text" :placeholder="$t('please_enter')" class="" id="purchase_order" 
    ...
    <el-dialog v-model="dialogVisible" :title="titleHeader" width="60%" draggable>
        <div class="container-fluid">
            <div class="row mx-2 mb-4">
                <el-table :data="orderData" border style="width: 100%;"  @row-click="handleSelectionChange">
                    <el-table-column fixed label="" width="60" >
                        <template #default="scope">
                            <el-radio-group v-model="selected_id" class="ml-4">
                                <el-radio :label="scope.row.purchase_order_id" size="large">{{ selected_name }}</el-radio>                                  
                              </el-radio-group>                              
                        </template>
                    </el-table-column>
        ...
        <template #footer>
          <span class="dialog-footer">
            ...
            <el-button type="primary" @click="selectOrderName" >{{ $t('sure') }}</el-button>           
          </span>
        </template>
    </el-dialog>
</template>
        ...
        async getOrderData(){               
            ...
            if(response.status == 200){ 
                ...
                if(this.orderData != null){
                    ...
                    if(value.purchase_order_id == this.oldOrderId){
                        this.selected_id = value.purchase_order_id;
         ...
         selectOrderName(){
            if(this.selected_id != null && this.selected_id != undefined && this.selected_id != ""){    
                this.title = this.orderData.find(x=> x.purchase_order_id == this.selected_id).title;         
                this.$emit("orderData", this.selected_id);
                this.dialogVisible = false;
            }  
                     
        }, 
        handleSelectionChange(val){          
           this.selected_id = val.purchase_order_id;
        },
