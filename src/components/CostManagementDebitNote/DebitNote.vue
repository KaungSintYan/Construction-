    ...
    <div class="container-fluid"  @click="$store.commit('setActive', false)">
        ...
        <div class="row mx-2">  
         <el-table :data="data" border show-summary sum-text="su..." style="width: 100%" @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="total_loan_amount" :label="$t('total_loan_amount')" width="200" sortable />
            <!-- <el-table-column prop="borrower" :label="$t('borrower')" width="200" sortable /> -->
            <el-table-column prop="borrowing_date" :label="$t('borrowing_date')" width="200" sortable />
            ...
            <template #default="scope">
             <div class="action__btn d-flex">
                            ...
                            <router-link  :to="`/debit_note/show/${scope.row.debit_note_id}`"
                                v-show="showBtn">
                                <el-tooltip
                                    class="box-item"
                                    effect="dark"
                            ...
                            <!-- <router-link :to="`/debit_note/edit/${scope.row.debit_note_id}`"
                            ...
                            placement="top"
                            >
                            <button class="btn-danger circle__btn" @click="deleteAll(scope.row.debit_note_id)" v-show="deleteBtn && scope.row.editable">
                            ...
                            placement="top"
                            >
                            <button class="btn-warning circle__btn"  @click="showApprover(scope.row.debit_note_id)">
...
export default {
    ...
    data() {
        return {   
            //searching
            apiName: '/costmanagement/get_debit_note',
            advanced: true,            
            routeName: "/debit_note/",   
            showDebitNote: true,        
            //endsearching
    ...
    methods:{     
        ...
        async showApprover(id){ 
            let response = await authApi({
                method: 'GET',
                api: '/costmanagement/get_debitnote_approve_cc',
                params: {
                    debit_note_id: id,                  
                }   
            });          
        ...
        async deleteAll(id = null){     
            ...
            if(id != null && id != ""){
                ...
                for(let i=0; i< this.selectedData.length; i++){
                      list.push(this.selectedData[i].debit_note_id);              
                }
            } 
