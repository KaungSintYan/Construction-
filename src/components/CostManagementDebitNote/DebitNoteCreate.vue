                    ...
                    <span v-if="v$.FormData.note.project_id.$error" class="text-danger">
                        <span v-if="v$.FormData.note.project_id.$errors[0].$message != ''">
                            ...
                            <div class="col-md-3 m-0 p-0">
                                <div class="form-group mb-3">
                                    ...
                                    id="borrowing_date" style="width: 100%;"  v-model="FormData.note.borrowing_date" clearable/>
                                    <span v-if="v$.FormData.note.borrowing_date.$error" class="text-danger">
                                        <span v-if="v$.FormData.note.borrowing_date.$errors[0].$message != ''">
                                            {{ $t('is_required',{'name': $t('borrowing_date')} ) }}
                                        </span>
                                    </span>
                                </div>
                            </div>

                            <!-- <Picker :titleName="$t('borrower')" @pickerData="getPickerName($event)"  /> -->
                        ...
                        <div class="row mx-2 mb-4" style="width: 100%;">
                                <el-table :data="FormData.details" border style="width: 100%;">
                                    ...
                                    <el-table-column :label="$t('loan_amount')" width="500px"> 
                                        <template #default="scope">
                                            <el-input type="number" :placeholder="$t('please_choose')" v-model="scope.row.loan_amount" clearable/>
                                        </template>
                                    </el-table-column>

                                    <el-table-column :label="$t('reason_for_borrowing')" width="500px"> 
                                        <template #default="scope">
                                            <el-input type="textarea" v-model="scope.row.reason_for_borrowing" :placeholder="$t('please_enter')" maxlength="100" autosize show-word-limit/>
                                        </template>
                                    </el-table-column>
        ...
        async createData(e){ 
            ...
            if(response.status == 201){              
              //console.log(response.data.revenue_contract_id);
                const id = response.data.debit_note_id; 
                const imgformData = new FormData();
                imgformData.append('debit_note_id',id);
                ...
                if(imgResponse.status == 200){
                    e.target.reset();
                    toast.success("Debit Note Insert successful !", {
