<template>  
   <div v-loading="loading" class="loading__div" v-if="loading"></div>   
    <div class="container-fluid"  @click="$store.commit('setActive', false)" v-if="!loading"> 
        <div class="row">
            <MaterialPlanSearching :api="apiName" :advanced="advanced"  :routeName="routeName" :createBtn="createBtn"
            :showInboundOrder="showInboundOrder"
        ...
        <div class="row mx-2">   
         <el-table :data="data" border style="width: 100%" @selection-change="handleSelectionChange">
            ...
            <!-- <el-table-column prop="enter_warehouse" :label="$t('warehouse_auto')" width="350" sortable /> -->
            <!-- <el-table-column prop="" :label="$t('material_details')" width="200" sortable /> -->
            <!-- <el-table-column prop="payment_status" :label="$t('payment_status')" width="200" sortable /> -->
            <el-table-column prop="purchase_order_id" :label="$t('purchase_order')" width="200" sortable />
            ...
            <el-table-column :label="$t('cc')" width="200" sortable >
             <template #default="scope">
                    <span v-for="(obj, index) in scope.row.cc" :key="index">
                            {{ obj }}
                        </span>
            </template>           
            </el-table-column>
            <el-table-column prop="application_date" :label="$t('applicat_data')" width="200" sortable />
            <!-- <el-table-column prop="picker" :label="$t('picker')" width="200" sortable />           -->
            ...
            <template #default="scope">
             <div class="action__btn d-flex">
                            <router-link :to="`/inbound_order/show/${scope.row.inbound_order_id}`"
                            v-show="showBtn">
                                <el-tooltip
                                    class="box-item"
                                    effect="dark"
                            ...
                            <!-- <router-link :to="`/inbound_order/edit/${scope.row.inbound_order_id}`"
                            ...
                            placement="top"
                            >
                            <button class="btn-danger circle__btn" @click="deleteAll(scope.row.inbound_order_id)" v-show="deleteBtn && scope.row.editable">
                            ...
                            placement="top"
                            >
                            <button class="btn-warning circle__btn"  @click="showApprover(scope.row.inbound_order_id)" >
...
import MaterialPlanSearching from '@/components/Share/MaterialPlanSearching.vue';
import $ from 'jquery';
export default {
    name: 'InboundOrder',
    data() {
        return {  
             //searching
             apiName: "/materialmanagement/get_inbound_order",
            advanced: true,            
            routeName: "/inbound_order/",
            showInboundOrder: true,
            //endsearching
    ...
    methods:{     
        ...
        async showApprover(id){ 
            let response = await authApi({
                method: 'GET',
                api: '/materialmanagement/get_inboundorder_approve_cc',
                params: {
                    inbound_order_id: id,                  
                }   
            });          
          ...
          async deleteAll(id = null){          
            ...
            if(id != null && id != ""){
                ...
                for(let i=0; i< this.selectedDatadata.length; i++){
                    list.push(this.selectedData[i].inbound_order_id);     
                }
            } 
            ...
            if(list.length > 0){
                let response = await authApi({
                method: 'DELETE',
                api: '/materialmanagement/inbound_order',
                params: {
                       id: '',
                       keyword: '',
                       status: '',                       
