<template>  
    <div class="container-fluid"  @click="$store.commit('setActive', false)">
        <div class="row">
            <ProjectSearching :api="apiName" :createBtn="createBtn" :routeName="routeName" :advanced="advanced" :showApply="showApply"
        ...
        <el-table :data="data" border style="width: 100%" @selection-change="handleSelectionChange">
            ...
            <!-- <el-table-column prop="applicant_id" :label="$t('applicant')" width="200" sortable /> -->
            <!-- <el-table-column :label="$t('printing_dep')" width="200" sortable >
            <template #default="scope">
                        <div class="container">
                            <div class="row" v-for="(dep, index) in  scope.row.printing_department" :key=index>
                                <span>{{ dep }}</span>                                 
                            </div>
                        </div>
            </template>
            </el-table-column> -->
            <el-table-column prop="print_file_name" :label="$t('print_file_name')" width="200" sortable />
            <el-table-column prop="number_of_copies" :label="$t('number_copies')" width="200" sortable />
            <el-table-column prop="printtype" :label="$t('print')" width="200" sortable />   
            <el-table-column prop="seal_type" :label="$t('seal_type')" width="200" sortable />
            ...
            <template #default="scope">
             <div class="action__btn d-flex">
                            <router-link  :to="`/sealapply/show/${scope.row.application_id}`"
                            v-show="showBtn">
                                <el-tooltip
                                    class="box-item"
                                    effect="dark"
                            ...
                            <!-- <router-link :to="`/sealapply/edit/${scope.row.application_id}`" 
                            ...
                            placement="top"
                            >
                            <button class="btn-danger circle__btn" @click="deleteAll(scope.row.application_id)" v-show="deleteBtn && scope.row.editable">
                            ...
                            placement="top"
                            >
                            <button class="btn-warning circle__btn"  @click="showApprover(scope.row.application_id)">
...
export default {
    ...
    data() {
        return { 
            //searching
            apiName: "/projectmanagement/getapplicationforprint",
            advanced: true,            
            routeName: "/sealapply/",
            showApply: true,
            //endsearching   
    ...
    methods:{   
        ...
        async showApprover(id){ 
            let response = await authApi({
                method: 'GET',
                api: '/projectmanagement/get_app_approve_cc',
                params: {
                    application_id: id,                  
                }   
            });
         ...
         async deleteAll(id = null){          
            ...
            if(id != null && id != ""){
                ...
                for(let i=0; i< this.selectedData.length; i++){
                     list.push(this.selectedData[i].application_id);  
                }
            } 
