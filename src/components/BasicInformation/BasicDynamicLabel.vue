<template>  
    ...
    <div class="container-fluid"  @click="$store.commit('setActive', false)" v-if="!loading"> 
        ...
        <div class="row mx-2">
            ...
            <el-table-column prop="label_name" :label="$t('name')" width="500" sortable />
            ...
            <template #default="scope">
                ...
                <el-button type="button" class="btn btn-sm btn-danger mr-2" @click="deleteAll(scope.row.label_id)">
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="form-group">
                    <label for="type_of_work"> {{ $t('dynamic_label') }}</label>
    ...
    <el-dialog v-model="dialogEditVisible" title="Edit job type" width="30%" center="true" draggable>     
        <div class="container-fluid">
            <div class="row">
                <div class="form-group">
                    <label for="type_of_work"> {{ $t('add_dynamic_tags') }}</label>
                    <el-input type="text" v-model="editFormData.label_name" :placeholder="$t('please_enter')" 
                    :class="{ error_border:errorMessage }" clearable />
    ...
    methods:{     
        ...
        async saveFormModel(){
            let response = await authApi({
                ...
                data: {
                    label_id: this.editFormData.label_id,
                    label_name: this.editFormData.label_name
                }
            });
            if(response.message){
                toast.success("dynamic label Update successful!", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 3000,
                });                
            ...
            if(this.formSearching != ""){
                ...
                if(response.message){   
                    ...
                    for(let i=0; i< list.length; i++){
                        let item= {"label_id": list[i].label_id,"label_name":list[i].label_name,"status": list[i].status, isSelected: false};
                        this.data.push(item)
                    }
        ...
        async createForm(e){
           ...
           if(this.formData.name == "" || this.formData.name == null){
            ...
            let response = await authApi({
                ...
                api: '/dynamiclabel',                 
                data:{
                    label_name: this.formData.name
                }
            });
            if(response.message){
                this.dialogVisible = false;
                toast.success("dynamic label insert successful !", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 3000,
                });
                this.getData();    
        ...
        async deleteAll(id = null){ 
            ...
            if(id != null && id != ""){
                ...
                for(let i=0; i< this.selectedData.length; i++){
                    list.push(this.selectedData[i].label_id);         
                }                
            } 
            ...
            if(list.length > 0){
                ...
                if(response.message){                   
                    toast.success("Dynamic Label Delete successful !", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 3000,
                    });
                    this.getData();
