<template>  
    ...
    <div class="container-fluid"  @click="$store.commit('setActive', false)" v-if="!loading"> 
        ...
        <div class="row mx-2">           
            <el-table :data="data" border style="width: 100%" @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="seal_type_name" :label="$t('name')" width="500" sortable />
                ...
                <el-table-column fixed="right" :label="$t('operate')"  width="500" >
                    <template #default="scope">
                        ...
                        <el-button type="button" class="btn btn-sm btn-danger mr-2" @click="deleteAll(scope.row.seal_type_id)">
    ...
    <!-- create model -->
    <el-dialog v-model="dialogVisible" :title="$t('new_stamp_type')" width="30%" center="true" draggable>     
        <div class="container-fluid">
            <div class="row">
                <div class="form-group">
                    <label for="type_of_work"> {{ $t('sealtype') }}</label>
                    <el-input type="text" v-model="formData.name" :placeholder="$t('please_enter')" 
                    :class="{ error_border:errorMessage }" clearable />
                    <span v-if="errorMessage" class="text-danger">Please enter the seal type</span>
                </div> 
    ...
    <!-- edit Model -->   
    <el-dialog v-model="dialogEditVisible" :title="$t('new_stamp_type')" width="30%" center="true" draggable>     
        <div class="container-fluid">
            <div class="row">
                <div class="form-group">
                    <label for="type_of_work"> {{ $t('sealtype') }}</label>
                    <el-input type="text" v-model="editFormData.seal_type_name" :placeholder="$t('please_enter')" 
                    :class="{ error_border:errorMessage }" clearable />
                    <span v-if="errorMessage" class="text-danger">Please enter the seal type name</span>
                </div>                 
            </div>
        </div>        
        <template #footer>
    ...
    methods:{
        ...
        async archive(items){ 
            ...
            let response = await authApi({
                ...
                data: {
                    seal_type_id: items.seal_type_id,
                    seal_type_name: items.seal_type_name,                  
                    status: archive
                }
            });
        ...
        async saveFormModel(){
            let response = await authApi({
                ...
                data: {
                    seal_type_id: this.editFormData.seal_type_id,
                    seal_type_name: this.editFormData.seal_type_name,   
                }
            });
            if(response.message){
                toast.success("seal type Update successful!", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 3000,
                });                
            ...
            if(this.formSearching != ""){
                ...
                if(response.message){   
                    ...
                    for(let i=0; i< list.length; i++){
                        let item= {"seal_type_id": list[i].seal_type_id,"seal_type_name":list[i].seal_type_name,
        ...
        async createForm(e){
           ...
           if(this.formData.name == "" || this.formData.name == null){
            ...
            let response = await authApi({
                ...
                api: '/sealtype',                 
                data:{
                    seal_type_name: this.formData.name,
                }
            });
            if(response.message){
        ...
        async deleteAll(id = null){ 
            ...
            if(id != null && id != ""){
                ...
                for(let i=0; i< this.selectedData.length; i++){
                    list.push(this.selectedData[i].seal_type_id);         
                }                
            } 
            ...
            if(list.length > 0){
                ...
                if(response.message){                   
                    toast.success("seal type Delete successful !", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 3000,
                    });
                    this.getData();
