<template>  
    ...
    <div class="container-fluid"  @click="$store.commit('setActive', false)" v-if="!loading"> 
        ...
        <div class="row mx-2">
            <el-table :data="data" border style="width: 100%" @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="section_name" :label="$t('section_name')" width="1000" sortable />
                ...
                <el-table-column fixed="right" :label="$t('operate')"  width="500" >
                    <template #default="scope">
                        ...
                        <el-button type="button" class="btn btn-sm btn-danger mr-2" @click="deleteAll(scope.row.section_type_id)">
    ...
    <el-dialog v-model="dialogEditVisible" :title="$t('new_section_type')" width="30%" center="true" draggable>     
        <div class="container-fluid">
            <div class="row">
                <div class="form-group">
                    <label for="type_of_section"> {{ $t('type_of_section') }}</label>
                    <el-input type="text" v-model="editFormData.section_name" :placeholder="$t('please_enter')" 
                    :class="{ error_border:errorMessage }" clearable />
    ...
    methods:{
        ...
        async archive(items){ 
            ...
            let response = await authApi({
                ...
                data: {
                    section_type_id: items.section_type_id,
        ...
        async saveFormModel(){
            let response = await authApi({
                ...
                data: {
                    section_type_id: this.editFormData.section_type_id,
                    section_name: this.editFormData.section_name,  
                }
            });
            if(response.message){
                toast.success("Section Type Update successful!", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 3000,
                });                
            ...
            if(this.formSearching != ""){
                ...
                if(response.message){   
                    ...
                    for(let i=0; i< list.length; i++){
                        let item= {"section_type_id": list[i].section_type_id,"section_name":list[i].section_name,
        ...
        async createForm(e){
           ...
           if(this.formData.name == "" || this.formData.name == null){
            ...
            let response = await authApi({
                ...
                api: '/projectmanagement/sectiontype',                 
                data:{
                    section_name: this.formData.name,
                }
            });
            if(response.message){
                this.dialogVisible = false;
                toast.success("Section Type insert successful!", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 3000,
                });
                this.getData();    
        ...
        async deleteAll(id = null){ 
            ...
            if(id != null && id != ""){
                ...
                for(let i=0; i< this.selectedData.length; i++){
                    list.push(this.selectedData[i].section_type_id);         
                }                
            } 
            ...
            if(list.length > 0){
                ...
                if(response.message){                   
                    toast.success("Section Type Delete successful !", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 3000,
                    });
                    this.getData();
