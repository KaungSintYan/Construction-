    ...
    <div class="container-fluid"  @click="$store.commit('setActive', false)" v-if="!loading"> 
        ...
        <div class="row mx-2">
            <el-table :data="data" border style="width: 100%" @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="typename" :label="$t('typename')" width="1000" sortable />
                <!-- <el-table-column prop="typecode" :label="$t('type_code')" width="1000" sortable /> -->
                ...
                <el-table-column fixed="right" :label="$t('operate')"  width="500" >
                    <template #default="scope">
                        ...
                        <el-button type="button" class="btn btn-sm btn-danger mr-2" @click="deleteAll(scope.row.type_id)">
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="form-group">
                    <label for="budget_type"> {{ $t('budget_type') }}</label>
                    <el-input type="text" v-model="FormData.budget.typename" :placeholder="$t('please_enter')" 
    ...
    <el-dialog v-model="dialogEditVisible" :title="$t('new_section_type')" width="30%" center="true" draggable>     
        <div class="container-fluid">
            <div class="row">
                <div class="form-group">
                    <label for="budget_type"> {{ $t('budget_type') }}</label>
                    <el-input type="text" v-model="editFormData.typename" :placeholder="$t('please_enter')" 
                    :class="{ error_border:errorMessage }" clearable />
    ...
    methods:{
        ...
        async saveFormModel(){
            let response = await authApi({
                ...
                data: {
                    type_id: this.editFormData.type_id,
                    typename: this.editFormData.typename,  
                }
            });
            if(response.message){
                toast.success("Budget Type Update successful!", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 3000,
                });                
            ...
            if(this.formSearching != ""){
                ...
                if(response.message){   
                    ...
                    for(let i=0; i< list.length; i++){
                        let item= {"type_id": list[i].type_id,"typename":list[i].typename,
        ...
        async createForm(e){
           e.preventDefault()            
           if(this.FormData.budget.typename == "" || this.FormData.budget.typename == null){
            ...
            let response = await authApi({
                ...
                api: '/projectmanagement/budgettype',                 
                data:{
                    typename: this.FormData.budget.typename,
                }
            });
            if(response.message){
                this.dialogVisible = false;
                toast.success("Budget Type insert successful!", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 3000,
                });
                this.getData();    
        ...
        async deleteAll(id = null){ 
            ...
            if(id != null && id != ""){
                ...
                for(let i=0; i< this.selectedData.length; i++){
                    list.push(this.selectedData[i].type_id);         
                }                
            } 
            ...
            if(list.length > 0){
                ...
                if(response.message){                   
                    toast.success("Budget Type Delete successful !", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 3000,
                    });
                    this.getData();
