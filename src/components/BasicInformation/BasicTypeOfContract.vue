<template>  
    ...
    <div class="container-fluid"  @click="$store.commit('setActive', false)" v-if="!loading"> 
        ...
        <div class="row mx-2">
            <el-table :data="data" border style="width: 100%" @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="contract_type_name" :label="$t('contract_type_name')" width="1000" sortable />
                ...
                <el-table-column fixed="right" :label="$t('operate')"  width="500" >
                    <template #default="scope">
                        ...
                        <el-button type="button" class="btn btn-sm btn-danger mr-2" @click="deleteAll(scope.row.contract_type_id)">
    ...
    <!-- create Model -->   
    <el-dialog v-model="dialogVisible" :title="$t('new_contract_type')" width="30%" center="true" draggable>     
        <div class="container-fluid">
            <div class="row">
                <div class="form-group">
                    <label for="type_of_contract"> {{ $t('type_of_contract') }}</label>
                    <el-input type="text" v-model="formData.name" :placeholder="$t('please_enter')" 
                    :class="{ error_border:errorMessage }" clearable />
                    <span v-if="errorMessage" class="text-danger">Please enter the type of contract</span>
                </div>                 
            </div>
        </div>        
        <template #footer>
    ...
    <!-- edit Model -->  
    <el-dialog v-model="dialogEditVisible" :title="$t('new_contract_type')" width="30%" center="true" draggable>     
        <div class="container-fluid">
            <div class="row">
                <div class="form-group">
                    <label for="type_of_contract"> {{ $t('type_of_contract') }}</label>
                    <el-input type="text" v-model="editFormData.contract_type_name" :placeholder="$t('please_enter')" 
                    :class="{ error_border:errorMessage }" clearable />
                    <span v-if="errorMessage" class="text-danger">Please enter the contract type</span>
                </div>                 
            </div>
        </div>        
        <template #footer>
    ...
    methods:{
        ...
        async archive(items){ 
            ...
            let response = await authApi({
                ...
                data: {
                    contract_type_id: items.contract_type_id,
                    contract_type_name: items.contract_type_name,                  
                    status: archive
                }
            });
        ...
        async saveFormModel(){
            let response = await authApi({
                ...
                data: {
                    contract_type_id: this.editFormData.contract_type_id,
                    contract_type_name: this.editFormData.contract_type_name,  
                }
            });
            if(response.message){
                toast.success("Type of Contract Update successful!", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 3000,
                });                
            ...
            if(this.formSearching != ""){
                ...
                if(response.message){   
                    ...
                    for(let i=0; i< list.length; i++){
                        let item= {"contract_type_id": list[i].contract_type_id,"contract_type_name":list[i].contract_type_name,
        ...
        async createForm(e){
           ...
           if(this.formData.name == "" || this.formData.name == null){
            ...
            let response = await authApi({
                ...
                api: '/contractmanagement/revenue_contract_type',                 
                data:{
                    contract_type_name: this.formData.name,
                }
            });
            if(response.message){
                this.dialogVisible = false;
                toast.success("Type of contract insert successful!", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 3000,
                });
                this.getData();    
        ...
        async deleteAll(id = null){ 
            ...
            if(id != null && id != ""){
                ...
                for(let i=0; i< this.selectedData.length; i++){
                    list.push(this.selectedData[i].contract_type_id);         
                }                
            } 
            ...
            if(list.length > 0){
                ...
                if(response.message){                   
                    toast.success("type of Contract Delete successful !", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 3000,
                    });
                    this.getData();
        ...
        async getData(){  
            ...
            if(response.status == 200 ){
                ...
                this.loading = false;
                console.log('contract'+ JSON.stringify(response.data))
            }
        },
