    ...
    <div class="container-fluid"  @click="$store.commit('setActive', false)" v-if="!loading"> 
        ...
        <div class="row mx-2">
            <el-table :data="data" border style="width: 100%" @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="source_name" :label="$t('source_name')" width="1000" sortable />
                ...
                <el-table-column fixed="right" :label="$t('operate')"  width="500" >
                    <template #default="scope">
                        ...
                        <el-button type="button" class="btn btn-sm btn-danger mr-2" @click="deleteAll(scope.row.source_id)">
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="form-group">
                    ...
                    <el-input type="text" v-model="editFormData.source_name" :placeholder="$t('please_enter')" 
                    :class="{ error_border:errorMessage }" clearable />
    ...
    methods:{
        ...
        async saveFormModel(){
            let response = await authApi({
                ...
                data: {
                    source_id: this.editFormData.source_id,
                    source_name: this.editFormData.source_name,  
                }
            });
            if(response.message){
                toast.success("Payment Source Update successful!", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 3000,
                });                
            ...
            if(this.formSearching != ""){
                ...
                if(response.message){   
                    ...
                    for(let i=0; i< list.length; i++){
                        let item= {"source_id": list[i].source_id,"source_name":list[i].source_name,
        ...
        async createForm(e){
           e.preventDefault()            
           if(this.FormData.payment.source_name == "" || this.FormData.payment.source_name == null){
            ...
            let response = await authApi({
                ...
                api: '/financialmanagement/payment_source',                 
                data:{
                    source_name: this.FormData.payment.source_name,
                }
            });
            if(response.message){
        ...
        async deleteAll(id = null){ 
            ...
            if(id != null && id != ""){
                ...
                for(let i=0; i< this.selectedData.length; i++){
                    list.push(this.selectedData[i].source_id);         
                }                
            } 
            ...
            if(list.length > 0){
                ...
                if(response.message){                   
                    toast.success("Salary Type Delete successful !", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 3000,
                    });
                    this.getData();
