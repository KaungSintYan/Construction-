    ...
    <div class="container-fluid"  @click="$store.commit('setActive', false)" v-if="!loading"> 
        ...
        <div class="row mx-2">
            <el-table :data="data" border style="width: 100%" @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="name" :label="$t('name')" width="1000" sortable />
                ...
                <el-table-column fixed="right" :label="$t('operate')"  width="500" >
                    <template #default="scope">
                        ...
                        <el-button type="button" class="btn btn-sm btn-danger mr-2" @click="deleteAll(scope.row.expenditure_category_id)">
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="form-group">
                    ...
                    <el-input type="text" v-model="editFormData.name" :placeholder="$t('please_enter')" 
                    :class="{ error_border:errorMessage }" clearable />
    ...
    methods:{
        ...
        async archive(items){ 
            ...
            let response = await authApi({
                ...
                data: {
                    expenditure_category_id: items.expenditure_category_id,
                    name: items.name,                  
                    status: archive
                }
            });
        ...
        async saveFormModel(){
            let response = await authApi({
                ...
                data: {
                    expenditure_category_id: this.editFormData.expenditure_category_id,
                    name: this.editFormData.name,  
                }
            });
            if(response.message){
                toast.success("Category of Expenditure Update successful!", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 3000,
                });                
            ...
            if(this.formSearching != ""){
                ...
                if(response.message){   
                    ...
                    for(let i=0; i< list.length; i++){
                        let item= {"expenditure_category_id": list[i].expenditure_category_id,"name":list[i].name,
        ...
        async deleteAll(id = null){ 
            ...
            if(id != null && id != ""){
                ...
                for(let i=0; i< this.selectedData.length; i++){
                    list.push(this.selectedData[i].expenditure_category_id);         
                }                
            } 
            ...
            if(list.length > 0){
                ...
                if(response.message){                   
                    toast.success("Category of Expenditure Delete successful !", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 3000,
                    });
                    this.getData();
