<template>  
    ...
    <div class="container-fluid"  @click="$store.commit('setActive', false)" v-if="!loading"> 
        ...
        <div class="row mx-2">
            <el-table :data="data" border style="width: 100%" @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="task_label_name" :label="$t('name')" width="1000" sortable />
                ...
                <el-table-column fixed="right" :label="$t('operate')"  width="500" >
                    <template #default="scope">
                        ...
                        <el-button type="button" class="btn btn-sm btn-danger mr-2" @click="deleteAll(scope.row.task_label_id)">
    ...
    <!-- edit Model -->  
    <el-dialog v-model="dialogEditVisible" :title="$t('new_task_label')" width="30%" center="true" draggable>     
        <div class="container-fluid">
            <div class="row">
                <div class="form-group">
                    <label for="type_of_work"> {{ $t('task_label') }}</label>
                    <el-input type="text" v-model="editFormData.task_label_name" :placeholder="$t('please_enter')" 
                    :class="{ error_border:errorMessage }" clearable />
    ...
    methods:{
        ...
        async archive(items){ 
            ...
            let response = await authApi({
                ...
                data: {
                    task_label_id: items.task_label_id,
                    task_label_name: items.task_label_name,                  
                    status: archive
                }
            });
        ...
        async saveFormModel(){
            let response = await authApi({
                ...
                data: {
                    task_label_id: this.editFormData.task_label_id,
                    task_label_name: this.editFormData.task_label_name,  
                }
            });
            if(response.message){
                toast.success("task label Update successful!", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 3000,
                });                
            ...
            if(this.formSearching != ""){
                ...
                if(response.message){   
                    ...
                    for(let i=0; i< list.length; i++){
                        let item= {"task_label_id": list[i].task_label_id,"task_label_name":list[i].task_label_name,
        ...
        async createForm(e){
           ...
           if(this.formData.name == "" || this.formData.name == null){
            ...
            let response = await authApi({
                ...
                api: '/tasklabel',                 
                data:{
                    task_label_name: this.formData.name,
                }
            });
            if(response.message){
                this.dialogVisible = false;
                toast.success("task label insert successful!", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 3000,
                });
                this.getData();    
        ...
        async deleteAll(id = null){ 
            ...
            if(id != null && id != ""){
                ...
                for(let i=0; i< this.selectedData.length; i++){
                    list.push(this.selectedData[i].task_label_id);         
                }                
            } 
            ...
            if(list.length > 0){
                ...
                if(response.message){                   
                    toast.success("task label Delete successful !", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 3000,
                    });
                    this.getData();
