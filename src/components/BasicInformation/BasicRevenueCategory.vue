<template>  
    ...
    <div class="container-fluid"  @click="$store.commit('setActive', false)" v-if="!loading">
        ...
        <div class="row mx-2">
            <el-table :data="data" border style="width: 100%" @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="revenue_category_name" :label="$t('name')" width="375" sortable />
                <el-table-column prop="attributes" :label="$t('revenue_and_expenditure_attribues')" width="375" sortable />
                ...
                <el-table-column fixed="right" :label="$t('operate')"  width="375" >
                <template #default="scope">
                    ...
                    <el-button type="button" class="btn btn-sm btn-danger mr-2" @click="deleteAll(scope.row.revenue_category_id)">
        ...
        <div class="container-fluid">
            <div class="row">
                <el-radio-group v-model="attribute">
                    <el-radio label="income" size="large"> {{ $t ('income_category') }}</el-radio>
                    <el-radio label="expenditure" size="large"> {{ $t('expenditure_category') }}</el-radio>
                </el-radio-group>
            </div>
            <div class="row">
                <div class="form-group">
                    <label for="type_of_work"> {{ $t('add_new_income_and_expanditure_category') }}</label>
    ...
    <el-dialog v-model="dialogEditVisible" title="Edit job type" width="30%" center="true" draggable>     
        <div class="container-fluid">
            <div class="row">
                <el-radio-group v-model="attribute">
                    <el-radio label="income" size="large" :checked="editFormData.attributes == 'income'"> {{ $t ('income_category') }}</el-radio>
                    <el-radio label="expenditure" size="large" :checked="editFormData.attributes != 'income'"> {{ $t('expenditure_category') }}</el-radio>
                </el-radio-group>
            </div>
            <div class="row">
                <div class="form-group">
                    <label for="type_of_work"> {{ $t('revenue_and_expenditure_attribues') }} </label>
                    <el-input type="text" v-model="editFormData.revenue_category_name" :placeholder="$t('please_enter')" 
                    :class="{ error_border:errorMessage }" clearable />
                    <span v-if="errorMessage" class="text-danger">Please enter the revenue category</span>
                </div>                 
            </div>
        </div>        
        <template #footer>
    ...
    methods:{      
        async archive(items){ 
            ...
            let response = await authApi({
                ...
                data: {
                    revenue_category_id: items.revenue_category_id,
                    revenue_category_name: items.revenue_category_name,
        ...
        editFormModel(items){
            ...
            this.attribute = items.attributes;         
        ...
        async saveFormModel(){
            let response = await authApi({
                ...
                data: {
                    revenue_category_id: this.editFormData.revenue_category_id,
                    revenue_category_name: this.editFormData.revenue_category_name,
                    attributes: this.attribute
                }
            });
            if(response.message){
                toast.success("revenue category Update successful!", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 3000,
                });                
            ...
            if(this.formSearching != ""){
                ...
                if(response.message){   
                    ...
                    for(let i=0; i< list.length; i++){
                        let item= {"revenue_category_id": list[i].revenue_category_id,"revenue_category_name":list[i].revenue_category_name,"attributes": list[i].attributes,
        ...
        async createForm(e){
           ...
           if(this.formData.name == "" || this.formData.name == null){
            ...
            let response = await authApi({
                ...
                api: '/revenuecategory',                 
                data:{
                    revenue_category_name: this.formData.name,
                    attributes: this.attribute
                }
            });
            if(response.message){
                this.dialogVisible = false;
                toast.success("revenue category insert successful !", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 3000,
                });
                this.getData();    
        ...
        async deleteAll(id = null){ 
            ...
            if(id != null && id != ""){
                ...
                for(let i=0; i< this.selectedData.length; i++){
                    list.push(this.selectedData[i].revenue_category_id);         
                }                
            }           
            if(list.length > 0){
                let response = await authApi({
                ...
                if(response.message){                   
                    toast.success("Revenue category Delete successful !", {
                    position: toast.POSITION.TOP_RIGHT,
                    autoClose: 3000,
                    });
                    this.getData();
