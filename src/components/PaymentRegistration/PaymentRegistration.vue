 ...
 <div v-loading="loading" class="loading__div" v-if="loading"></div>   
    <div class="container-fluid"  @click="$store.commit('setActive', false)">
        <div class="row">
              ...
              :showPaymentRegistration="showPaymentRegistration"           
    ...
    <div class="container-fluid"  @click="$store.commit('setActive', false)" v-if="!loading">
        ...
        <div class="row mx-2">    
          <el-table :data="data" border style="width: 100%" @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="payment_type" :label="$t('payment_type')" width="200" sortable />
            <el-table-column prop="amount" :label="$t('payment_amount')" width="200" sortable />
            <!-- <el-table-column prop="prepayment_amount" :label="$t('prepayment_offset_amount')" width="290" sortable /> -->
            <!-- <el-table-column prop="actual_amount" :label="$t('actual_payment_amount')" width="250" sortable />  -->
            <el-table-column prop="payment_source" :label="$t('payment_source')" width="200" sortable /> 
            <el-table-column prop="spending_contract_id" :label="$t('payment_contract')" width="200" sortable /> 
            <el-table-column prop="payment_request_id" :label="$t('payment_request')" width="200" sortable />         
            <!-- <el-table-column prop="" :label="$t('beneficiary')" width="200" sortable />          -->
            <!-- <el-table-column prop="payment_date" :label="$t('payment_date')" width="200" sortable /> -->
            ...
            <template #default="scope">
             <div class="action__btn d-flex">
                           <router-link :to="`/payment_registration/show/${scope.row.payment_registeration_id}`"
                            v-show="showBtn">
                                <el-tooltip
                                    class="box-item"
                                    effect="dark"
                            ...
                            <router-link :to="`/payment_registration/edit/${scope.row.payment_registeration_id}`"
                            ...
                            placement="top"
                            >
                            <button class="btn-danger circle__btn" @click="deleteAll(scope.row.payment_registeration_id)" v-show="deleteBtn && scope.row.editable">
                            ...
                            placement="top"
                            >
                            <button class="btn-warning circle__btn"  @click="showApprover(scope.row.payment_registeration_id)">
...
export default {
    ...
    data() {
        return {   
               //searching
            apiName: '/financialmanagement/get_payment_registration',
            advanced: true,            
            routeName: "/payment_registration/",
            showPaymentRegistration: true, 
            //endsearching
    ...
    methods:{ 
        ...
        async showApprover(id){ 
            let response = await authApi({
                method: 'GET',
                api: '/financialmanagement/get_payment_register_approve_cc',
                params: {
                    payment_registeration_id: id,                  
                }   
            });          
           ...
           async deleteAll(id = null){          
            ...
            if(id != null && id != ""){
                ...
                for(let i=0; i< this.selectedData.length; i++){
                    list.push(this.selectedData[i].payment_registeration_id);   
                }
            } 
