<template>  
    ...
    <div class="container-fluid"  @click="$store.commit('setActive', false)" v-if="!loading"> 
        <div class="row">
            <ContractSearching :api="apiName" :advanced="advanced"  :routeName="routeName" :createBtn="createBtn"
        ...
        <el-table :data="data" border style="width: 100%" @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="project_id" :label="$t('project')" width="200" />
            <el-table-column prop="contract_title" :label="$t('contract_title')" width="200" sortable />
            <el-table-column prop="approve_status" :label="$t('approval_status')" width="200" sortable />
            <el-table-column prop="contract_no" :label="$t('contract_no')" width="200" sortable />
            <el-table-column prop="contract_type_id" :label="$t('type_of_contract')" width="200" sortable />          
            <el-table-column prop="amount_with_tax" :label="$t('amount_including_tax')" width="200" />
            ...
            <el-table-column prop="date_of_signing" :label="$t('date_of_signing')" width="200" sortable />
            <!-- <el-table-column prop="total_visa_amount" :label="$t('visa_amount')" width="200" sortable /> -->
            <!-- <el-table-column prop="currency" :label="$t('currency')" width="200" sortable /> -->
            <!-- <el-table-column prop="total_billing_app_amount" :label="$t('billing_app_amount')" width="270" sortable />  -->
            <!-- <el-table-column prop="total_reciept_register_amount" :label="$t('receipt_registration_amount')" width="250" sortable />  -->
            <el-table-column prop="start_date" :label="$t('start_date')" width="200" sortable />         
            <el-table-column prop="end_date" :label="$t('end_date')" width="200" sortable />         
            ...
            <template #default="scope">
             <div class="action__btn d-flex">
                            <router-link :to="`/revenue/show/${scope.row.revenue_contract_id}`"
                            v-show="showBtn">
                                <el-tooltip
                                    class="box-item"
                                    effect="dark"
                            ...
                            <!-- <router-link :to="`/revenue/edit/${scope.row.revenue_contract_id}`"
                            ...
                            placement="top"
                            >
                            <button class="btn-danger circle__btn" @click="deleteAll(scope.row.revenue_contract_id)" v-show="deleteBtn && scope.row.editable">
                            ...
                            placement="top"
                            >
                            <button class="btn-warning circle__btn"  @click="showApprover(scope.row.revenue_contract_id)">
...
import $ from 'jquery';
import ContractSearching from '@/components/Share/ContractSearching.vue';
export default {
    name: 'Revenue',
    data() {
        return {     
             //searching
            apiName: "/contractmanagement/get_revenue_contract",
            advanced: true,            
            routeName: "/revenue/",
            //endsearching
            data:[],
    ...
    methods:{ 
        ...
        async deleteAll(id = null){          
            ...
            if(id != null && id != ""){
                ...
                for(let i=0; i< this.selectedData.length; i++){
                    list.push(this.selectedData[i].revenue_contract_id); 
                }
            } 
            ...
            if(list.length > 0){
                ...
                if(response.message){
                    ...
                    this.getData();
                    toast.success("Revenue Delete successful !", {
        ...
        async showApprover(id){ 
            let response = await authApi({
                method: 'GET',
                api: '/contractmanagement/get_rc_approve_cc',
                params: {
                    contract_id: id,                  
                }   
            });          
