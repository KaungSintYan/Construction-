    ...
    <div class="container-fluid"  @click="$store.commit('setActive', false)">      
        ...
        <div class="row mx-2">   
            <el-table :data="data" border style="width: 100%">
                ...
                <el-table-column prop="" :label="$t('lease_type')" width="200" sortable />
                <el-table-column prop="" :label="$t('implementation_modalities')" width="250" sortable />
...
export default {
    ...
    methods:{ 
        async getData(val){            
            let type= "";
            let api= '/contractmanagement/sp_approver_cc';            
            if(val == "CC"){               
                type = 0;
        ...
        getEdit(data){
        this.FormData.contract_id= data.spending_contract_id;
        ...
        async updateData() {
            let response = await authApi({
                method: 'POST',
                api: '/contractmanagement/sp_approver_cc',
                params: {
                       id: '',
                       type: 1,
                },
